...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	Если Не ДанныеДоговора.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбоненскоеОбслуживание Тогда
		...
		Сообщение = "Для данного вида обслуживания должен быть выбран договор абонентского обслуживания";
	...
	Если НачалоДня(ДанныеДоговора.ВКМ_ДатаНачала) > НачалоДня(ДатаПроведенияРабот) 
		...
		Сообщение = "Дата проведения работ должна находится в периоде действия договора";
		ОбщегоНазначения.СообщитьПользователю(Сообщение);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ПроцентОтРабот = Неопределено Тогда
			...
			Сообщение = "Не заполнены данные регистра ""Условия оплаты сотрудников"" на дату проведения документа.";
			ОбщегоНазначения.СообщитьПользователю(Сообщение);
		...
		Движение.СуммаКОплате = Движение.ЧасовКОплате * ДанныеДоговора.ВКМ_СтимостьЧаса * Выборка.ПроцентОтРабот / 100;
