...
Процедура ПроверитьИзменениеДокумента(Документ, МассивИзменений)
	...
	Если Не Выборка.ВремяНачалаРабот = ВремяНачалаРабот Тогда
		МассивИзменений.Добавить(СтрШаблон("Время начала работ с %1 на %2", Выборка.ВремяНачалаРабот, ВремяНачалаРабот));
	КонецЕсли;
	
	Если Не Выборка.ВремяОкончанияРабот = ВремяОкончанияРабот Тогда
		МассивИзменений.Добавить(СтрШаблон("Время окончания работ с %1 на %2", Выборка.ВремяОкончанияРабот, ВремяОкончанияРабот));
	КонецЕсли;
