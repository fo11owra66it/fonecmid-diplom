...
Процедура СформироватьДвиженияПоОтпускам()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
			...
			Движение.БазовыйПериодКонец = НачалоМесяца(ДобавитьМесяц(Выборка.ДатаОкончания, -1));	
		КонецЕсли;
...
Процедура РассчитатьУдержания()
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.СуммаБаза, 0) КАК ОснСуммаБаза,
		|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.СуммаБаза, 0) КАК ДопСуммаБаза,
		|	ВКМ_ПроцентНДФЛ.Значение КАК ПроцентНДФЛ,
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Сумма = (Выборка.ОснСуммаБаза + Выборка.ДопСуммаБаза) * Выборка.ПроцентНДФЛ / 100;
	КонецЦикла;
